...
public:
  ...
  struct InstalledModel {
    ...
    std::string filename;
    bool managed = true;
    // Optional cached metadata for managed models (populated when installed)
    std::optional<RemoteModel> metadata;
  };
  ...
  bool import_model(const std::filesystem::path &source, std::string &error, std::filesystem::path *out_path = nullptr);
