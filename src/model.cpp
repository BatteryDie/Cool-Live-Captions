...
bool extract_json_bool(const std::string &json, std::string_view key, bool fallback) {
  auto pos = json.find(key);
  ...
  if (json.compare(end, 4, "true") == 0) return true;
  if (json.compare(end, 5, "false") == 0) return false;
  return fallback;
}
...
void ModelManager::load_installed() {
  ...
  while (true) {
    ...
    bool managed = extract_json_bool(chunk, "\"managed\"", true);
    if (!id.empty() && !filename.empty()) {
      InstalledModel im{version, filename, managed};
      if (managed) {
        // try to extract cached metadata if present
        std::string name = extract_json_field(chunk, "\"name\"");
        std::string author = extract_json_field(chunk, "\"author\"");
        std::string language = extract_json_field(chunk, "\"language\"");
        std::string description = extract_json_field(chunk, "\"description\"");
        std::string url = extract_json_field(chunk, "\"url\"");
        std::string url_website = extract_json_field(chunk, "\"url_website\"");
        std::uint64_t size_bytes = extract_json_uint(chunk, "\"size_bytes\"");
        if (!name.empty() || !author.empty() || !language.empty() || !description.empty() || !url.empty() || !url_website.empty() || size_bytes != 0) {
          RemoteModel rm;
          rm.id = id;
          rm.version = version;
          ...
          rm.url = url;
          rm.url_website = url_website;
          rm.size_bytes = size_bytes;
          im.metadata = std::move(rm);
        }
      }
      installed_[id] = std::move(im);
    }
    pos = brace_end + 1;
  }
}
...
void ModelManager::save_installed() const {
  ...
  for (const auto &kv : installed_) {
    ...
    out << "  {\"id\":\"" << escape(kv.first) << "\",\"version\":\"" << escape(kv.second.version)
      << "\",\"filename\":\"" << escape(kv.second.filename) << "\",\"managed\":"
      << (kv.second.managed ? "true" : "false");
    if (kv.second.managed && kv.second.metadata.has_value()) {
      const auto &m = kv.second.metadata.value();
      out << ",\"name\":\"" << escape(m.name) << "\"";
      out << ",\"author\":\"" << escape(m.author) << "\"";
      out << ",\"language\":\"" << escape(m.language) << "\"";
      out << ",\"description\":\"" << escape(m.description) << "\"";
      out << ",\"url\":\"" << escape(m.url) << "\"";
      out << ",\"url_website\":\"" << escape(m.url_website) << "\"";
...
void ModelManager::record_install(const RemoteModel &remote, const std::filesystem::path &local_path) {
  {
    std::lock_guard<std::mutex> lock(installed_mutex_);
    InstalledModel im{remote.version, local_path.filename().string(), true};
    im.metadata = remote;
    installed_[remote.id] = std::move(im);
  }
  save_installed();
}

bool ModelManager::import_model(const std::filesystem::path &source, std::string &error, std::filesystem::path *out_path) {
  if (!std::filesystem::exists(source)) {
    error = "Selected file does not exist.";
    return false;
  }
  if (!is_supported(source)) {
    error = "Unsupported model file type.";
    return false;
  }
  ensure_dir(user_dir_);

  auto filename = source.filename();
  auto target = user_dir_ / filename;
  if (std::filesystem::exists(target)) {
    auto stem = target.stem().string();
    auto ext = target.extension().string();
    for (int i = 1; i < 1000; ++i) {
      auto candidate = user_dir_ / (stem + " (" + std::to_string(i) + ")" + ext);
  ...
  {
    std::lock_guard<std::mutex> lock(installed_mutex_);
    std::string id = std::string("local:") + target.filename().string();
    installed_[id] = InstalledModel{"local", target.filename().string(), false};
  }
  save_installed();
  if (out_path) {
    *out_path = target;
...
void ModelManager::sync_installed_with_disk() {
  std::vector<std::filesystem::path> files = enumerate(user_dir_);
  std::vector<std::string> available;
  available.reserve(files.size());
  for (const auto &p : files) {
    if (is_supported(p)) {
      available.push_back(p.filename().string());
    }
  }
  ...
  bool changed = false;
  {
    ...
    for (auto it = installed_.begin(); it != installed_.end();) {
      if (!std::binary_search(available.begin(), available.end(), it->second.filename)) {
